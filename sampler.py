...
class ResShiftSampler(BaseSampler):
    ...
    def inference(self, in_path, out_path,
                  mask_path=None, mask_back=True, bs=1, noise_repeat=False,
                  dataset=None):
        '''
        Inference demo.
        Input:
        ...
        if in_path.is_dir():
            if dataset is None:
                raise ValueError("Dataset must be provided for directory input.")
            ...
            for data in dataloader:
                ...
                if micro_data['lq'].shape[0] > 0:
                    ...
                    for jj in range(results.shape[0]):
                        experiment = micro_data['filename'][jj].split('/')[-3]
                        filename = micro_data['filename'][jj].split('/')[-1]
                        # if not os.path.exists(os.path.join(out_path,'gt',experiment,
                        #                                       )):
                        #     for folder in ['gt','pred','input']:
                        #         os.mkdir(os.path.join(out_path, folder, experiment))
                        #         os.mkdir(os.path.join(out_path, folder, experiment,'Images'))


                        im_name = os.path.join(experiment,'Images',filename)

                        # ## save predicted image
                        # im_sr = util_image.tensor2img(results[jj], rgb2bgr=False, min_max=(results[jj].min().cpu(), 1.0))
                        # save_path = os.path.join(out_path,'images',im_name)
                        # for idx_channel,channel in enumerate([1,2,3,4,5,]):
                        #     save_name = save_path +  '-ch%isk1fk1fl1.tiff' % (channel)
                        #     img_for_save = im_sr[...,idx_channel]
                        #     imwrite(save_name, img_for_save)

                        ## save local image
                        im_sr = util_image.tensor2img(results[jj], rgb2bgr=False,
                                                      min_max=(results[jj].min().cpu(), 1.0))
                        save_path = os.path.join(out_path,im_name)
                        for idx_channel,channel in enumerate([1,2,3,4,5,]):
                            save_name = save_path +  '-ch%isk1fk1fl1.tiff' % (channel)
                            img_for_save = im_sr[...,idx_channel]
                            imwrite(save_name, img_for_save)
                        ...
                        # ## save input image
                        # im_sr = util_image.tensor2img(micro_data['lq'], rgb2bgr=False, min_max=(-1.0, 1.0))
                        # save_path = os.path.join(out_path,'input',im_name)
                        # for idx_channel,channel in enumerate([1,2,3,4,5]):
                        #     save_name = save_path +  '-ch%isk1fk1fl1.tiff' % (idx_channel+6)
                        #     img_for_save = im_sr[...,idx_channel]
                        #     imwrite(save_name, img_for_save)
                        #
                        # ## save gt image
                        # im_sr = util_image.tensor2img(micro_data['gt'], rgb2bgr=False, min_max=(0, 1.0))
                        # save_path = os.path.join(out_path,'gt',im_name)
                        # for idx_channel,channel in enumerate([1,2,3,4,5,9,10]):
                        #     save_name = save_path +  '-ch%isk1fk1fl1.tiff' % (channel)
                        #     img_for_save = im_sr[...,idx_channel]
                        #     imwrite(save_name, img_for_save)
