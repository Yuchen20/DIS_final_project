...
class TrainerBase:
    def __init__(self, configs):
        ...
        if self.rank == 0:
            wandb.init(
                project="cell-painting-diffusion",
                config=OmegaConf.to_container(configs),
                name=f"{configs.get('experiment_name', 'default')}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    def logging_image(self, im_tensor, tag, phase, add_global_step=False, nrow=8):
        ...
        if self.local_logging:
            ...
            channels = [im_tensor[i] for i in range(im_tensor.shape[0])]
            im_tensor = torch.cat(channels, dim=1).unsqueeze(0)
        ...
        if self.rank == 0:
            wandb.log({
                f"{phase}/{tag}": wandb.Image(im_tensor.cpu()),
                "step": self.log_step_img[phase]
            })
        if add_global_step:
    ...
    def logging_metric(self, metrics, tag, phase, add_global_step=False):
        ...
        if self.rank == 0:
            if isinstance(metrics, dict):
                wandb_metrics = {f"{phase}/{tag}/{k}": v for k, v in metrics.items()}
        else:
                wandb_metrics = {f"{phase}/{tag}": metrics}
            wandb_metrics["step"] = self.log_step[phase]
...
class TrainerDifIR(TrainerBase):
    ...
    def build_model(self):
        ...
        if self.configs.autoencoder is not None:
            try:
                ckpt = torch.load(self.configs.autoencoder.ckpt_path, map_location=f"cuda:{self.rank}")['state_dict']
            ...
            self.autoencoder = IdentityFirstStage()
    ...
    def log_step_train(self, loss, tt, batch, z_t, z0_pred, phase='train'):
        if self.rank == 0:
            ...
            if self.current_iters % self.configs.train.log_freq[0] == 0:
                ...
                metrics = {
                    'loss': {f't{record_steps[jj]}': self.loss_mean['loss'][jj].item() for jj in range(len(record_steps))},
                    'mse': {f't{record_steps[jj]}': self.loss_mean['mse'][jj].item() for jj in range(len(record_steps))},
                    'lr': self.optimizer.param_groups[0]['lr']
                }
                self.logging_metric(metrics, 'Loss', phase, add_global_step=True)
            ...
            if self.current_iters % self.configs.train.save_freq == 0:
                ...
                if self.rank == 0:
                    wandb.log({"train/elapsed_time": elaplsed, "step": self.current_iters})
    ...
    def validation(self, phase='val'):
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                if 'gt' in data:
                    num_channels = sample_decode['sample'].size(1)
                    channels = [sample_decode['sample'][:,i] for i in range(num_channels)]
                    concatenated_tensor = torch.cat(channels, dim=1).unsqueeze(0).repeat(1,3,1,1)

                    num_channels = im_gt.size(1)
                    channels = [im_gt[:,i] for i in range(num_channels)]
                    concatenated_tensor_gt = torch.cat(channels, dim=1).unsqueeze(0).repeat(1,3,1,1)

                    mean_psnr += util_image.batch_PSNR(
                            concatenated_tensor * 0.5 + 0.5,
                            concatenated_tensor_gt * 0.5 + 0.5,
                            ycbcr=self.configs.train.val_y_channel,
                            )

                    total_loss = 0.0
                    batch_size, num_channels, height, width = im_gt.size()

                    for ch in range(num_channels):
                        x0_pred_rgb = sample_decode['sample'][:, ch:ch + 1, :, :].repeat(1, 3, 1, 1).clamp(-1.0, 1.0)
                        gt_rgb = im_gt[:, ch:ch + 1, :, :].repeat(1, 3, 1, 1)
                        channel_loss = self.lpips_loss(x0_pred_rgb, gt_rgb)
                        total_loss += channel_loss

                    total_loss /= num_channels
                    mean_lpips += total_loss.sum().item()
            ...
            if 'gt' in data:
                ...
                self.logging_metric(mean_psnr, 'PSNR', phase, add_global_step=False)
                self.logging_metric(mean_lpips, 'LPIPS', phase, add_global_step=True)
...
class TrainerDifIRLPIPS(TrainerDifIR):
    def backward_step(self, dif_loss_wrapper, micro_data, num_grad_accumulate, tt):
        ...
        with context():
            ...
            # classification loss
            num_channels = x0_pred.shape[1]
            gt = micro_data['gt']
            total_loss = 0
            for ch in range(num_channels):
                x0_pred_rgb = x0_pred[:, ch:ch + 1, :, :].repeat(1, 3, 1, 1).clamp(-1.0, 1.0)
                gt_rgb = gt[:, ch:ch + 1, :, :].repeat(1, 3, 1, 1)

                # Calculate LPIPS loss for the expanded image
                channel_loss = self.lpips_loss(x0_pred_rgb, gt_rgb)
                total_loss += channel_loss

            # Normalize by the number of channels
            total_loss /= num_channels
            losses["lpips"] = total_loss.squeeze(0).squeeze(0).squeeze(0)
