...
class OriDataset2(Dataset):
    def __init__(self, configs):
        ...
        self.data_root = configs['dir_paths']
    ...
    def __getitem__(self, idx):
        ...
        # Load label channels (original: [6,6,6,6,6])
        for i in [6,6,6,6,6]:
        ...
        # Load output channels (original: [1,2,3,4,5])
        output = []
        for i in range(1, 6):
            img = imread(os.path.join(base_dir, f"{prefix}-ch%isk1fk1fl1.tiff" % (i)))
            img = self.normalize(img)
...
class OriDataset2_3channel(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load label channels (original: [6,7,8])
        for i in [6,7,8]:
            img = imread(os.path.join(base_dir, f"{prefix}-ch%isk1fk1fl1.tiff" % (i)))
            img = self.normalize(img)
            img_preprocessed = self.transform(Image.fromarray(img))
        ...
        # Load output channels (original: [1,2,5])
        output = []
        for i in [1,2,5]:
            img = imread(os.path.join(base_dir, f"{prefix}-ch%isk1fk1fl1.tiff" % (i)))
            img = self.normalize(img)
